...
def topological_sort(graph):
    ...
    ready = sorted(
        [node for node in graph if count[node] == 0 and not graph[node]])
    # print
    # print 'ready'
    # print ready
    result = []
    ...
    near_leafs.sort(
        key=lambda x: len(
            graph[x]))  # sort by number of successors
    # print
    # print 'near_leafs'
    # print near_leafs
...
class Member:
    ...
    def __init__(self, ofs, size, alignment, leaf_type,
                 contents, index, suppress_meta=False):
        '''
    ...
    def to_string(self, tab):
        if self.suppress:
            ...
            return '{0:<50} {1}'.format(
                tab + self.member_str(), self.comment_str())
    ...
    def comment_str(self):
        ...
        if self.leaf_type == 'LF_BITFIELD':
            ...
            # ofs is an int, show it in hex too
            if self.ofs != 0 and int(self.ofs) == self.ofs:
...
def get_tpname(lf, name=None):
    if isinstance(lf, str):
        ...
        tpname = ptr_str(lf, name)
    elif lf.leaf_type == "LF_PROCEDURE":
        tpname = proc_str(lf, name)
    elif lf.leaf_type == "LF_MODIFIER":
        tpname = mod_str(lf, name)
    elif lf.leaf_type == "LF_ARRAY":
        tpname = arr_str(lf, name)
    elif lf.leaf_type == "LF_BITFIELD":
        tpname = bit_str(lf, name)
    else:
        tpname = lf.leaf_type
        if name:
            tpname += " " + name
...
def arr_str(arr, name):
    ...
    if sz < 0:
        print >>sys.stderr, "ERROR with array %s %s -- element size is negative" % (
            tpname, name)
    if arr.size < 0:
        print >>sys.stderr, "ERROR with array %s %s -- size is negative" % (
            tpname, name)
    count = arr.size / sz
...
def simplify_union(union):
    ...
    """
    # if last structure is bigger, its fields are probably not part of the
    # union
    last_field = union[-1]
...
def merge_bitfield_sequences(members):
    ...
    while i < len(members):
        ...
        if is_bitfield(curr):
            ...
            if in_bitfield:
                in_bitfield = False
                bf_member.size = get_aligned_size(
                    bf_member.size,
                    bf_member.alignment)
                bf_member.contents += '};'
...
def fill_gaps(lf, members, mbr_ct_by_ofs):
    ...
    while ofs < lf.size:
        try:
            ...
            # now there's one member in this range.
            mbr_ct_by_ofs[ofs:j] = [1] * gap_size
...
def unionize_compute(lf, member_list):
    ...
    if s.computed_size != s.claimed_size:
        ...
        new_mlist.append('*/')
        new_mlist.append(
            'UINT8 blob[0x%x]; // print_ctypes.py validation failure' %
            lf.size)
...
def flstr(lf):
    ...
    enum_membs = [
        e for e in lf.fieldlist.substructs if e.leaf_type == "LF_ENUMERATE"]
    for i, e in enumerate(enum_membs):
        e_val = -1 if e.enum_value == '\xff' else e.enum_value
        comma = ",\n" if i < len(enum_membs) - 1 else ""
        # indent + "%s = %s%s\n" % (e.name, e_val, comma)
        flstr += '{0}{1:<70} = {2:>4}{3}'.format(indent, e.name, e_val, comma)
    return flstr
...
def struct_pretty_str_nofwd(lf, gcc):
    ...
    if gcc:
        print_to_output(
            "} __attribute__((packed)) %s, *P%s, **PP%s ;" %
            ((demangle(
                lf.name),
              ) *
                3))
...
if __name__ == "__main__":
    ...
    if opts.width:
        ...
        try:
            ...
            if pdb.STREAM_DBI.machine in ('IMAGE_FILE_MACHINE_I386'):
                ...
                sys.stderr.write(
                    "Failed to find arch pointer width. Use the -w option.")
                raise
    ...
    if opts.macroguard:
        macroguard_str = "_WINDOWS_PDB_" + \
            os.path.basename(args[0]).replace('.', '_') + "_defns"
    ...
    for s in structs:
        names[s.name] = s
        if decl_names and s.name not in decl_names:
            continue
    ...
    for e in enums:
        if e not in sorted_structs_name:
            continue  # file based filter
        enum_pretty_str(e)

    print_to_output("/*******  Structures  *******/")
    for n in sorted_structs_name:
        if n not in names.keys():
            continue  # probably enum
        s = names[n]
        if "unnamed" in s.name:
            continue
        if s.leaf_type == "LF_ENUM":
            continue
