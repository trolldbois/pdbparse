...
def bit_str(bitf):
    return "bitfield pos: %d len: %d [%s]" % (
        bitf.position, bitf.length, get_tpname(bitf.base_type))
...
structs = [
    s for s in pdb.streams[2].types.values() if (
        s.leaf_type == "LF_STRUCTURE" or s.leaf_type == "LF_UNION") and not s.prop.fwdref]
enums = [e for e in pdb.streams[
    2].types.values() if e.leaf_type == "LF_ENUM" and not e.prop.fwdref]
